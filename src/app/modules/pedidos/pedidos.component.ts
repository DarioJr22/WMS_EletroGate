...
import { Situacoes, situacoes } from 'src/app/services/itens';
...
import { Observable, catchError, concatMap, delayWhen, forkJoin, from, interval, map, merge, mergeMap, of, switchMap, timer, BehaviorSubject } from 'rxjs';
...
export class PedidosComponent implements OnInit {
  ...
  options: any;
  pedido: any;
  situacoes: BehaviorSubject<Situacoes[]> = new BehaviorSubject(situacoes);
  ...
  getInitialValues(){
    const situacoes = this.situacoes.value
    let paramsChat = this.fillParamsFilter([
      ...situacoes.map(sit => sit.id)
    ]);
    let paramsTable = this.fillParamsFilter([
      ...situacoes.map(sit => sit.id)
    ]);
  ...
  getSituacoes(idModule: number) {
    this.pedidoServ.getSituations(idModule).subscribe({
      next: (res: any) => {
      this.situacoes.next([...res.data as Situacoes[]])
  ...
  getSitucaoStl(id: number) {
    let situacao = this.situacoes.value.find((i) => i.id == id);
    if (situacao) {
  ...
  reloadTable() {
    this.dadosFilter = [];
    const situacoes = this.situacoes.value
    let params = this.fillParamsFilter([...situacoes.map(sit => sit.id)]);
    this.getPedidos(params, 'table');
  }
  ...
  createChart() {
    ...
    const {
      ...
      this.getValuesGrafico(this.dados, this.situacoes.value);
